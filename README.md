...
At the end of this documentation, you will have a configured systemd service running DCI jobs with the latest versions of RHOSP or RDO.
dci-openstack-agent is an ansible-playbook configured as a systemd service. This service will run on a RHEL server called remoteci.
...
To be able to work with DCI you **must have** automation scripts for undercloud and overcloud deployment.
You should be able to automatically:

- clean and redeploy the undercloud machine automatically
- install the undercloud using repository and registry urls as parameter
...
You be able to install the rpm of DCI Ansible Agent, you will need to activate some extra repositories and add EPEL and DCI repositories.
...
Here the structure of the configuration files for dci-openstack-agent
...
DCI hooks are ansible tasks that will be run by the agent. You need to call your clean and provisionning script in the pre-run.yml task.
...
dci-openstack-agent will start a job by downloading the lastest component based on the topic version setted in `/etc/dci-openstack-agent/settings.yml`
...
Then the dci-openstack-agent will create a local repository (`http://<dci_mirror_location>/dci_repo/dci_repo.repo`) with the rpm downloaded and create a local registry (`<dci_mirror_location>:5000`) for containers.
...
At the end of the running tasks, you **must** add the undercloud ip to the dynamic ansible inventory
...
You are here? This is a good new because you have a working automated agent running on your remoteci. We can know automate your server to run job automatically

### Configure your time source

Having an chronized clock is important to get meaningful log files. This is the reason why the agent ensure Chrony is running. 
...
Distributed-CI is a rolling release. We publish updates every with and we expect our users to keep their jumpbox up to date.
