...
for instance to run half of your deployments with an option and the other half without the option.

This feature is not available yet on the interface. You need to use the `dcictl` CLI tool for that.

First, you need to identify the ID of your remoteci and the topic.
```console
$ dcictl topic-list
$ dcictl remoteci-list
```

With these information, you can now associate the `rconfiguration` to the remoteci.
