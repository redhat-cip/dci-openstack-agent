...
RUN yum upgrade -y && \
  ...
  yum -y install gcc ansible python python2-devel python2-pip chrony openssh && \
  yum -y install ansible-role-dci-import-keys ansible-role-dci-retrieve-component dci-ansible && \
  yum clean all
