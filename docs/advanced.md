...
## Tempest: How to limit the tests executed to a subset

The agent runs by default all the available tests. This can be very long and in
 some cases can have produce false negatives or unstable results if your
 environment does not match the test use-case.
...
A frequent practice is to run only the "smoke" tests subset:
```console
